...
def rdw():
    ...
    if not k: return jsonify({"success":False,"message":"Geen kenteken opgegeven."})
    try:
        url="https://opendata.rdw.nl/resource/m9d7-ebf2.json"
        resp=requests.get(url, params={"kenteken":k}, timeout=8)
        data=resp.json() if resp.ok else []
        if not data: return jsonify({"success":False,"message":"Kenteken niet gevonden bij RDW."})
        row=data[0]; merk=row.get("merk",""); handels=row.get("handelsbenaming","")
    ...
    except Exception as e: return jsonify({"success":False,"message":f"RDW fout: {e}"})
...
<br><label>Merk</label><input id="merk" name="merk">
...
if(kentekenEl){ kentekenEl.addEventListener('change',()=>{
 fetch('/format_kenteken',{method:'POST',headers:{'Content-Type':'application/json'},
  body:JSON.stringify({raw:kentekenEl.value||""})})
 .then(r=>r.json()).then(d=>{if(d&&d.formatted) kentekenEl.value=d.formatted;});});}
async function haalRdw(){const val=(kentekenEl&&kentekenEl.value||'').trim(); if(!val){alert('Vul kenteken in');return;}
 try{const resp=await fetch('/rdw?kenteken='+encodeURIComponent(val.replace(/[^A-Za-z0-9]/g,'')));
  const d=await resp.json(); if(d&&d.success){document.getElementById('merk').value=d.merk||"";
   document.getElementById('type').value=d.type||"";document.getElementById('bouwjaar').value=d.bouwjaar||"";}
  else{alert(d&&d.message?d.message:'Geen gegevens.');}}catch(e){alert('Fout bij RDW');}}
...
@app.post("/submit")
def submit():
    form=request.form; kenteken=format_kenteken(form.get("kenteken",""))
    ...
    c.drawString(2*cm,y,"Opdrachtbon"); y-=1*cm
    for line in [kenteken,merk,vtype,bouwjaar]: c.drawString(2*cm,y,line); y-=0.7*cm
    c.showPage(); c.save(); pdf_bytes=buf.getvalue(); filename="bon.pdf"
    recipients=[]; 
    if admin_list: 
    ...
    if sender and recipients:
        try:
            msg=build_message("Opdrachtbon", "Bijlage opdrachtbon", sender, ", ".join(recipients),
                attachments=[(pdf_bytes, filename, "application/pdf")])
            if klantemail: msg['Reply-To']=klantemail
            status=send_email(msg); print("[mail] ok:",status)
        except Exception as e: print("[mail][error]",e)
    buf.seek(0); return send_file(buf,as_attachment=True,download_name=filename,mimetype="application/pdf")

@app.get("/mail_test")
def mail_test():
    to=request.args.get("to") or os.getenv("RECEIVER_EMAIL") or os.getenv("SENDER_EMAIL")
    sender=os.getenv("SENDER_EMAIL") or ""
    if not to or not sender: return jsonify({"ok":False,"error":"SENDER_EMAIL of ontvanger ontbreekt"}),400
    try:
        msg=build_message("Testmail","Dit is een test",sender,to)
        status=send_email(msg); return jsonify({"ok":True,"status":status}),200
